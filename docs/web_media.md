...
| Downloader | `Web/WebMediaDownloader.cs` | `UnityWebRequest` to memory (images, 25 MB cap) or file (videos, 250 MB cap), browser User-Agent, 30 s timeout, `Abort()` on cap/cancel, magic-byte sniffing, public-host-only URL gate (`IsAllowedPublicHttpUrl`: http/https, no userinfo, no loopback / RFC1918 / link-local / `localhost` with trailing dots normalized, no raw quote / backslash / whitespace / control characters), redirects followed by hand (`redirectLimit = 0`, max 5 hops, each `Location` re-gated; UnityWebRequest used to follow a public URL's 302 into loopback) |
...
not a bot"), then `ffmpeg normalize: 12-17s of source, 832x468, audio, audio normalized -22.4 -> -16 LUFS -> clip_....mp4 (1.2 MB)` and
