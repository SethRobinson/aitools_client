...
- `source_chat_image="N"` / `source_attachment="N"` - the **source**
...
Chain - composite the just-generated image into a slot on the prior canvas:

```
<aitools_action skill="generate_image" preset="{{Prompt To Image (Z-Image).txt}}" prompt="<full description for panel content>"/>
<aitools_action skill="paste_image" chain="true" source_chat_image="3" x="0" y="50%" width="50%" height="50%" mode="fill"/>
```

Note: in the chain example, the chain target is the canvas (the prior
Pic in this reply). The just-generated image is referenced via
`source_chat_image` - chain="true" never affects the source slot.
