...
- UP TO 3 STANDALONE AUDIO REFERENCES: `audio="N"` (then `audio2`, `audio3`)
  ...
  stays quoted ("she says, her voice styled like <Audio 1>: 'exact line'");
  a speech verb with no quoted words makes H3 invent dialog in a random
  language (tested 2026-08-30). Every staged audio ref's tag must appear in
  the prompt (same host gate as photos). Up to 3 standalone audio refs per
  render (separate from clip soundtracks).
