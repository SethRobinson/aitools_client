...
public class AIGuideManager : MonoBehaviour
{
    ...
    private void SpawnOpenAIRequest(Queue<GTPChatLine> lines, LLMProviderSettings activeSettings = null)
    {
        ...
        RTDB db = new RTDB();
        RTConsole.Log("Contacting OpenAI at " + profile.endpoint + " with " + json.Length + " bytes...");
        _openAITextCompletionManager.SpawnChatCompleteRequest(json, OnGTP4CompletedCallback, db, apiKey, profile.endpoint, OnStreamingTextCallback, true);
    }
    ...
    private void SpawnOpenAICompatibleRequest(Queue<GTPChatLine> lines, LLMProviderSettings activeSettings = null)
    {
        ...
        float compatTemperature = (settings != null && settings.overrideTemperature)
            ...
            : (isDeepSeek ? LLMRequestProfile.GetRecommendedTemperature(model, compatReasoning.effort, m_extractor.Temperature) : m_extractor.Temperature);
