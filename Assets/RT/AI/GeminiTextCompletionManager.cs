...
public class GeminiTextCompletionManager : MonoBehaviour
{
    ...
    public string BuildChatCompleteJSON(Queue<GTPChatLine> lines, int max_tokens = LLMRequestProfile.NoExplicitOutputTokenCap, float temperature = 1.0f, string model = "", bool stream = false, bool enableThinking = true, bool includeTemperature = true)
    {
        ...
        if (includeTemperature) // false = let Gemini use its model default
