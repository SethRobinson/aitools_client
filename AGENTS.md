...
- AI Chat starts with a local welcome bubble and hides local `Info`/debug bubbles by default. AI Chat Settings has a `Show debug stuff` checkbox (default off) that makes those bubbles visible; hidden recap-eligible Info messages still queue for the next LLM turn.
