...
- The `image_to_movie` skill autoloads for image-to-video/animation requests and advertises `Image To Video (WAN) 5s.txt` as the user-facing WAN/Wan 2.2 preset name. With the `test_` preset prefix active, that resolves to `test_Image To Video (WAN) 5s.txt`, the test WAN image-to-video path; the older `Image To Video (Wan22).txt` name remains a legacy alias. The test WAN image-to-video presets use the no-default-upscale AIO3 path, then run built-in `FL_RIFE` 4x interpolation and save at 64 fps, so a 1024 source does not automatically become a 2048 video.
