...
- Stop: `ShouldStopBeInteractable()` in `AIChatPanel.cs` is the single Stop-enable rule (streaming, forced-main wait, inspections, pending auto-resumes, web work, audio generation); `SetBusyUI` must never compute its own. Known gap: the video-import gate (`_videoImportCount`: clip imports, `set_video_audio`, `stitch_video` waits) blocks Send but has no Stop path.
...
**Prefer this automation bridge for Unity/editor validation whenever it is enabled.** Before launching a separate Unity batch/editor process, check `GET /status`; if the bridge is ready, use it to compile (`/rebuild`), drive the app (`/play`, `/stop`, `/settings`, `/llm_settings`, `/open_chat`, `/chat`, `/chat_import_video`, `/click`, `/scroll`, ...) and capture evidence (`/screenshot`, `/save`, `/chat_images`, `/movie_state`). For UI/AI Chat work, verify with screenshots whenever possible rather than relying only on code inspection. Do not start a second Unity editor/batchmode instance against this project while the editor lockfile/server is active unless the bridge is unavailable or the task specifically requires batchmode.
